#!/usr/bin/env python
#
# t2t - patched txt2tags generic text conversion tool
# http://pst.libre.lu/toolz/t2t
#
# Copyright 2001, 2002, 2003, 2004, 2005, 2006 Aurelio Marinho Jargas
# Copyright 2005, 2006 Pascal Steichen
#
...
# pst:	added ltb to TARGETS and TARGET_NAMES
# 	ltb which stands for latex-beamer ;)
TARGETS  = ['html', 'xhtml', 'sgml', 'tex', 'ltb', 'lout', 'man', 'mgp',
            'moin', 'pm6'  , 'txt'] 
#pst: added handout flag
...
_("      --handout       (only ltb format) create handouts of slideshows (default OFF)"),
...
\usepackage[%(ENCODING)s]{inputenc}  %% char encoding
\frenchspacing      %% no aditional spaces after periods
\usetheme{%(STYLE)s} %% user defind style
...
def getTags(config):
	...
	# pst: ltb inherits all not found below from tex ;)
	'ltb': {
	   'title1Open'        	: '\n\\section*{\a}\n\\begin{frame}\n\\frametitle{\a}',
	   'title1Close'       	: '\\end{frame}',
	   'title2Open'        	: '\n\\subsection*{\a}\n\\begin{frame}\n\\frametitle{\\strut}\n\\framesubtitle{\a}',
	   'title2Close'	: '\\end{frame}',
	   'numtitle1Open'     	: '\n\\section{\a}\n\\begin{frame}\n\\frametitle{\a}',
	   'numtitle1Close'    	: '\\end{frame}',
	   'numtitle2Open'     	: '\n\\subsection{\a}\n\\begin{frame}\n\\frametitle{\\strut}\n\\framesubtitle{\a}',
	   'numtitle2Close'	: '\\end{frame}',
	   'listOpen'		: '\\begin{itemize}[<+->]',
	   'numlistOpen'        : '\\begin{enumerate}[<+->]',
	   'deflistOpen'         : '\\begin{description}[<+->]',
	# verbatim and quotations aren't handle very well
	# so using blockquote instead
	   'blockVerbOpen'      : '\\begin{quotation}[<+->]\n\\texttt{',
	   'blockVerbClose'     : '}\n\\end{quotation}',
	   'TOC'                : '\\frame{\\tableofcontents}'
	},
	...
	# pst: do the tex -> ltb inheritance
	ltb = alltags['tex'].copy()
	ltb.update(alltags['ltb'])
        alltags['ltb'] = ltb.copy()
...
def getRules(config):
	...
	# pst: new rules needed for the ltb target
	  'titleblocksnodepth',	  # title section blocks do not use depth feature
	...
	rules_bank = {
	  ...
	  # pst: ltb inherits all except the following from tex ;)
	  'ltb': {
	    'titleblocks':1,
	    'titleblocksnodepth':1,
	    'listmaxdepth':3,
	    'verbblocknotescaped':0
	    },
	  'lout': {
	    'keepquoteindent':1,
	    'escapeurl':1,
	...
	elif config['target'] == 'ltb':
		myrules = rules_bank['tex'].copy()   # inheritance
		myrules.update(rules_bank['ltb'])    # get ltb specific
	else:
...
class TitleMaster:
	...
	def _open_close_blocks(self):
		...
		curr = self.level
		if rules['titleblocksnodepth']: curr = last
	...
	# pst: new func for ltb level specifics
	def close_last(self):
		"Closes last opened title (ltb specific)"
		ret = []
		ret.extend(self.tag_hold)
		tag = TAGS.get('title%dClose'%self.level)
		if tag: ret.append(tag)
...
def doHeader(headers, config):
	...
	#pst: compose handouts if needed
	template = doHandout(config)
	...
	for i in 0,1,2:
		...
		# pst: added ltb
		if (target != 'tex' or target != 'ltb') or \
		  ((target == 'tex' or target == 'ltb') and re.search(r'\\\w+{', contents)):
...
#pst: new function to create handouts of ltb slideshow
def doHandout(config):
	target = config['target']
	if not HEADER_TEMPLATE.has_key(target):
		Error("doHandout: Unknow target '%s'"%target)
	template = HEADER_TEMPLATE[target]

	# change documentclass to use handout option
	if target == 'ltb' and config['handout']:
		Debug('template before: %s'%template)
		template = template.replace('\\documentclass{beamer}','\\documentclass[handout]{beamer}')
		Debug('template after: %s'%template)
...
def doFooter(config):
	...
	typename = target
	# pst: added ltb
	if target == 'ltb': typename = 'LaTeX2e with latex-beamer class'
...
def convert(bodylines, config, firstlinenr=1):
	...
	# pst: use ltb specific title closing ;)
	if rules['titleblocks']:
		if TARGET == 'ltb':
			ret.extend(TITLE.close_last())
		else:
